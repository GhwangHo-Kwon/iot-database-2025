...
/* 문제3 - 부서에 담당하는 업무를 한 번씩만 출력하시오.(20행)
*/
...
/* 샘플 - hr부서 예산 편성 문제로 급여 정보 보고서를 작성한다. employees에서 salary가 7000 ~ 10000달러 범위 이외의 사람의
		성과 이름을 Name으로 하여 급여를 급여가 작은 순서로 출력하시오.(75행)
*/
select concat(first_name, ' ', last_name) as 'name'
	 , salary
  from employees
...
/* 문제2 - 현재 날짜 타입을 날짜함수를 통해 확인, 1995년 5월 20일 ~ 1996년 5월 20일 사이 입사한 사원의 이름을 Name으로 별칭
...
-- 단일행 함수 및 변환 함수
/* 문제1 - 이름이 s로 끝나는 각 사원의 업무를 아래의 예와 같이 출력하고자함.(18행)
...
/* 문제3 - 사원의 성과 이름을 Name으로 별칭, 입사일, 입사한 요일을 출력하시오. 이때 주(week)의 시작인 일요일부터 출력되도록 정렬(107행)
*/
select concat(first_name, ' ', last_name) as 'Name'
	 , hire_date
...
select department_id
 ...
 group by department_id;  -- group by에 속한 컬럼만 select절에 사용할 수 있음!
...
/* 문제3 - 각 사원의 상사와의 관계를 이용하여 보고서 작성을 하려고 함
			예를 보고 출력하시오.(107행)
*/
