...
/* 샘플문제1 - 프로젝트 성공으로 급여인상이 결정.
	사원은 현재 107명 19개 업무에 소속되어 근무중. 회사 업무중 Distince job_id의 5가지 업무에서 일하는 사원.
    HR_REP(10%), MK_REP(12%), PR_REP(15%), SA_REP(18%), IT_PROG(20%). 5가지 업무를 제외하고는 나머지는 동결(107행)
*/
select employee_id
...
*/
-- 형변환 함수 cast(), convert()
select cast('-09' as unsigned);  -- unsigned(양수만나타내는숫자형)
select convert('-09', signed);  -- signed(음수포함숫자형태)
...
/* 샘플 - 부서와 업무별 급여합계를 구하고 신년도 급여수준 레벨을 지정하려고 함.
			부서 번호와 업무를 기본으로 정해 행을 그룹별로 나누어 급여합계와 인원수를 출력(20행)
*/
select department_id, job_id
	 , concat('$', format(sum(salary), 0)) as 'Salary_SUM'
...
select department_id, job_id
  ...
  from employees
 group by department_id, job_id
  with rollup;  -- group by의 컬럼이 하나면 총계는 하나, 컬럼이 두개면 첫번째 컬럼별로 소계, 두 컬럼의 합산이 총계로 나옴
...
select employee_id
     ...
     , dense_rank() over (partition by department_id order by salary desc) as 'Dense_Rank'   -- 1, 1, 2 순위 매기기 rank
