...
from tkinter.font import *  # 기본 외 폰트를 사용
...
# 5. DB처리 함수들 정의
## 5-1. showDatas() - DB 학생정보 테이블의 데이터를 가져와 TreeView에 표시
def showDatas():
    '''
    데이터베이스 내 모든 학생정보를 가져와 표시하는 사용자함수\n
    매개변수 필요없음
    '''
    global dataView   # 외부에 있는 변수
...
def getData(event):
    ...
    if sel_item:
        item_values = dataView.item(sel_item, 'values')  # 선택항목의 'values'(실데이터) 가져오기
    
    ## 엔트리 위젯에 각각 정보 채워넣기
    ent1.config(state='normal')     # 데이터가 들어갈 수 있게 해주고
    ent1.insert(0, item_values[0])  # 학생번호
    ent1.config(state='readonly')   # 다시 readonly로 설정
...
def addData():
    ...
    showDatas()     # DB 테이블의 모든 데이터 조회 후 트리뷰에 표시
...
def delData():
    ...
    if std_id == '':
        messagebox.showwarning('DELETE', '삭제 데이터를 선택요망!')
        return
...
## 4-5. 트리뷰 설정
for col in cols:
    dataView.heading(col, text=col)                 # 각열의 제목을 cols에 변수 하나씩 지정
...
# 6. 트리뷰항목을 더블클릭하면 이벤트발생(getData()함수 호출)
